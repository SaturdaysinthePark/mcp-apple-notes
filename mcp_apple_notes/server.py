...
async def list_all_notes(ctx: Context) -> str:
    """List all notes across all folders with their names, IDs, folder locations, and dates.

    Features:
    - Lists ALL notes from ALL folders in Apple Notes
    - Shows note names, IDs, folder locations, creation and modification dates
    - Provides comprehensive system overview

    Output Format:
    - Numbered list of all notes
    - Note name, ID, folder, creation date, and modification date
...
async def find_notes_by_title(
    ...
    title: str = Field(
        ...,
        description="Text to search for in note titles (case-insensitive contains match by default)",
    ),
    exact: bool = Field(
        default=False,
        description="If True, requires an exact case-sensitive title match. Default is False (contains search).",
    ),
) -> str:
    """Find notes whose title contains (or exactly matches) the given text.

    Features:
    - Searches note titles directly — no need to know the note ID first
    - Case-insensitive contains search by default
    - Optional exact match mode
    - Returns note ID, folder, creation date, and modification date

    Output Format:
    - Numbered list of matching notes with full metadata
    - Use the returned note_id + name with read_note to fetch content

    Returns:
        List of notes whose title matches the search text
    """
    try:
        results = await notes_tools.find_notes_by_title(title, exact)

        if not results:
            match_type = "exactly matching" if exact else "containing"
            return f"No notes found with title {match_type}: '{title}'"

        result = f"Notes matching title '{title}' ({len(results)} found):\n\n"
        ...
        await ctx.error(f"Error finding notes by title: {str(e)}")
        raise
...
async def find_notes_by_date(
    ...
    date_type: str = Field(
        default="modified",
        description="Which date to filter on: 'created' or 'modified' (default: 'modified')",
    ),
    after: str = Field(
        default="",
        description="ISO 8601 date string (e.g. '2024-01-15'). Include notes on or after this date. Leave empty for no lower bound.",
    ),
    before: str = Field(
        default="",
        description="ISO 8601 date string (e.g. '2024-12-31'). Include notes on or before this date. Leave empty for no upper bound.",
    ),
) -> str:
    """Find notes filtered by creation or modification date.

    Features:
    - Filter by 'created' or 'modified' date
    - Supports open-ended ranges (after only, before only, or both)
    - Results sorted newest first
    - Returns note ID, folder, and both dates for each match

    Examples:
    - Notes modified this week: date_type='modified', after='2024-01-15'
    - Notes created in January: date_type='created', after='2024-01-01', before='2024-01-31'
    - Notes modified before a date: date_type='modified', before='2024-06-01'

    Returns:
        List of notes matching the date filter, sorted newest first
    """
    try:
        results = await notes_tools.find_notes_by_date(date_type, after, before)
        ...
        if not results:
            ...
            return f"No notes found with {date_type} date {date_desc}"
        ...
        result = f"Notes {date_type} {date_desc} ({len(results)} found):\n\n"
        for i, note in enumerate(results, 1):
            result += f"{i:3d}. {note.get('name', 'N/A')}\n"
            result += f"     ID: {note.get('note_id', 'N/A')}\n"
            result += f"     Folder: {note.get('folder', 'N/A')}\n"
        ...
        await ctx.error(f"Error finding notes by date: {str(e)}")
        raise
